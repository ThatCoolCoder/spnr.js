if(window.spnr!==undefined){spnr.internalWarn('An instance of spnr.js is already running');}
else{var spnr={};spnr.VERSION='v1.4.0';spnr.consoleLogHeader='  🔧🔧 ';spnr.consoleLogStyling='background-color: #9cc8ff; display: block';window.spnr=spnr;console.log(`%c  \n${spnr.consoleLogHeader} spnr.js ${spnr.VERSION}  \n  `,spnr.consoleLogStyling);Object.getOwnPropertyNames(Math).forEach(key=>{spnr[key]=Math[key];});}
...
spnr.GameEngine=class{static pixiApp;static canvasSize;static globalPosition;static globalAngle;static globalScale;static crntScene;static deltaTime;static init(canvasSize,globalScale,backgroundColor=0x000000){spnr.internalWarn('spnr.GameEngine is an undocumented, untested festure. Use with caution');this.globalPosition=spnr.v(0,0);this.globalAngle=0;this.setGlobalScale(globalScale);this.createPixiApp(canvasSize,backgroundColor);this.deselectCrntScene();this.keyboard=new spnr.KeyWatcher();this.mouse=new spnr.MouseWatcher(this.pixiApp.view);}
...
spnr.GameEngine.ParticleEffect=class extends spnr.GameEngine.Entity{timer=0;playing=false;particlesRemaining=false;constructor(name,localPosition,localAngle,emitterData,looping=false){super(name,localPosition,localAngle);this.emitterData=emitterData;this.looping=looping;}
...
else{if(this.looping)this.play()
